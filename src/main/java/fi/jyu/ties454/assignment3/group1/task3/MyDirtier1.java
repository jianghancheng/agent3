...
public class MyDirtier1 extends GameAgent {
	...
	protected void setup() {
		...
		this.addBehaviour(new OneShotBehaviour() {
			...
			public void action() {
				Random r = new Random();
				Optional<AreaDirtier> areaDirtier = MyDirtier1.this.getDevice(DefaultDevices.AreaDirtier.class);
				Optional<JumpForwardMover> jumper = MyDirtier1.this.getDevice(DefaultDevices.JumpForwardMover.class);
				if (jumper.isPresent()) {
					jumper.get().move();
					jumper.get().move();
					while (true) {
